...
const NameLabel = styled.div`
    text-align: center;
    padding: 5px;
    background-color: ${({theme, isZero}) => isZero ? theme.colors.grey : theme.colors.text};
    color: white;
    border-radius: 5px;
`
...
const CountLabel = styled.div`
    ...
    right: 5px;
    padding: 5px;
    background-color: ${({theme, isZero}) => isZero ? theme.colors.grey : theme.colors.base};
    color: white;
    border-radius: 5px;
`
...
export default function MyBagItem({data}: MyBagItemProps) {
    ...
    const isTexture = useMemo<boolean>(() => ['wallpaper', 'tile'].includes(data.category), [data]);
    ...
    return <MyBagItemContainer isZero={isZero}>
        {
            isTexture
                ? <MyBagItemTextureImg
                    isZero={isZero}
                ...
                : <Canvas key={`my-bag-${data.id}`}
                    ...
                    />
                    <FurnitureModel key={`my-bag-${data.id}`}
                                    data={{...data, position: [0, 0, 0], rotation: [0, 0, 0]}}/>
                </Canvas>
        }
        <NameLabel isZero={isZero}>
            {data.name}
        </NameLabel>
        <CountLabel isZero={isZero}>
            {data.count}
        </CountLabel>
    </MyBagItemContainer>
}